...
def daily_report(report_date: date = Query(...), db: Session = Depends(get_db)):
    return report_service.daily_report(db, report_date)


@router.get("/weekly")
def weekly_report(report_date: date = Query(...), db: Session = Depends(get_db)):
    return report_service.weekly_report(db, report_date)


@router.get("/monthly")
def monthly_report(month: str = Query(..., pattern=r"^\d{4}-\d{2}$"), db: Session = Depends(get_db)):
    return report_service.monthly_report(db, month)


@router.get("/daily/export/excel")
def export_daily_excel(report_date: date = Query(...), db: Session = Depends(get_db)):
    rows = report_service.daily_report(db, report_date)
...
def export_daily_pdf(report_date: date = Query(...), db: Session = Depends(get_db)):
    rows = report_service.daily_report(db, report_date)
...
def export_weekly_excel(report_date: date = Query(...), db: Session = Depends(get_db)):
    rows = report_service.weekly_report(db, report_date)
...
def export_weekly_pdf(report_date: date = Query(...), db: Session = Depends(get_db)):
    rows = report_service.weekly_report(db, report_date)
...
def export_monthly_excel(month: str = Query(..., pattern=r"^\d{4}-\d{2}$"), db: Session = Depends(get_db)):
    rows = report_service.monthly_report(db, month)
...
def export_monthly_pdf(month: str = Query(..., pattern=r"^\d{4}-\d{2}$"), db: Session = Depends(get_db)):
    rows = report_service.monthly_report(db, month)
